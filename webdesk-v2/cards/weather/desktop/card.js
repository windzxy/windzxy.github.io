...
export async function mount({host,sdk,signal}){
  ...
  host.innerHTML=`<section class="weather-v2 weather-v2-desktop"><div><small>${data.place}${data.isFallback?' · 預覽':''}</small><h3>${weatherIcon(data.code)} ${data.temperature}°</h3><p>${weatherLabel(data.code)} · 體感 ${data.feelsLike}°</p></div><div class="weather-v2-grid"><span>最高 ${data.high}°</span><span>最低 ${data.low}°</span><span>降雨 ${data.rain}%</span><span>風速 ${data.wind} km/h</span></div><button type="button" data-open-weather>查看完整天氣</button></section>`;
