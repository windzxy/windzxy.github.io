...
export async function mount({host,signal}){
  async function render(location=null){
    ...
    host.innerHTML=`<section class="weather-app weather-app-tablet">
      ...
      <div class="weather-tablet-days" aria-label="七日預報">${d.forecast.slice(0,7).map(x=>`<article><span>${x.label}</span><strong>${weatherIcon(x.code)}</strong><small>${x.rain}%</small><b>${x.low}°</b><em>${x.high}°</em></article>`).join('')}</div>
      <div class="weather-tablet-sheet" data-sheet hidden><div class="weather-tablet-search"><input class="wd-search" data-query type="search" placeholder="搜尋城市" autocomplete="off"><button class="ios-button ios-button-secondary" data-close>完成</button></div><div data-results></div></div>
    </section>`;bind();
  }
