...
export async function mount({host,sdk,signal}){
  ...
  host.innerHTML=`<section class="weather-v2 weather-v2-mobile"><div class="weather-v2-mobile-top"><div><small>${data.place}${data.isFallback?' · 預覽':''}</small><strong>${weatherIcon(data.code)} ${data.temperature}°</strong></div><span>${weatherLabel(data.code)}</span></div><div class="weather-v2-mobile-meta"><span>${data.high}° / ${data.low}°</span><span>降雨 ${data.rain}%</span></div><button type="button" data-open-weather>查看天氣</button></section>`;
