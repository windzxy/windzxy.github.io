(function(){
  ...
  const APP='metals';
  const VER='20260819-gold-widget32-refresh-button-fix';
  const SNAP_MS=200;
  const DEFAULT_W=620,DEFAULT_H=410,MIN_W=300,MIN_H=238,CHART_W=310,CHART_H=260;
  const PRODUCTS=[
  ...
  function render(card={}){
    ...
    return `<div class="metals-widget mdesk ${cls}" data-version="${E(VER)}" data-product="${E(p.id)}">
      <header class="md-head"><div class="md-feed"><i class="${q.ok&&!stale?'on':''}"></i><b>${feed==='api'?'API':'LIVE'}</b></div><button class="md-refresh" data-metals-refresh title="刷新">↻</button></header>
      <nav class="md-tabs">${PRODUCTS.map(tab).join('')}</nav>
      <section class="md-chart"><div class="md-title"><em>${E(p.sym)}</em><b>${E(p.name)} K線</b></div><div class="md-tv" data-tv-symbol="${E(p.tv)}"><div class="md-loading">TradingView 圖表載入中…</div></div></section>
      ...
      <footer class="md-foot"><button data-metals-open="${E(p.url)}">源頁</button></footer>
    </div>`;
  }
  ...
  function timeLabel(q){return q.sourceTime||q.t||'';}
  ...
  function initApi(){
    ...
    try{PRODUCTS.forEach(p=>fn.call(sdk,{quoteParam:p.sym},d=>{const x=parseApi(d,p);if(x)put(p,x,'','api');}));paint();return true;}catch(e){console.warn(e);feed='snapshot';return false;}
  }
  async function refresh(force){
    if(feed==='api'&&!force)return;if(busy){pending=!!force;return;}busy=true;
    try{const data=await loadQuotes();PRODUCTS.forEach(p=>data[p.code]?put(p,data[p.code],'','snapshot'):Object.assign(quotes[p.id],{ok:false,status:'',t:clock(),ts:Date.now()}));}
    catch(e){console.warn(e);PRODUCTS.forEach(p=>Object.assign(quotes[p.id],{ok:false,status:'',t:clock(),ts:Date.now()}));}
  ...
  function paint(){PRODUCTS.forEach(p=>document.querySelectorAll(`[data-price-tab="${p.id}"]`).forEach(e=>e.textContent=F((quotes[p.id]||{}).price)));const q=quotes[product().id]||blank(product());[['price',q.price,0],['change',q.change,1],['pct',q.pct,1,'%'],['buy',q.buy,0],['sell',q.sell,0],['spread',spread(q),0],['high',q.high,0],['low',q.low,0],['prev',q.prev,0],['open',q.open,0],['low2',q.low,0],['high2',q.high,0]].forEach(a=>document.querySelectorAll(`[data-active-${a[0]}]`).forEach(e=>e.textContent=a[2]?S(a[1],a[3]||''):F(a[1])));const ok=is(q.price)&&is(q.low)&&is(q.high)&&q.high>q.low,pc=ok?Math.max(0,Math.min(100,(q.price-q.low)/(q.high-q.low)*100)):50;document.querySelectorAll('[data-active-range]').forEach(e=>e.style.left=pc+'%');document.querySelectorAll('[data-metals-time]').forEach(e=>e.textContent=timeLabel(q));}
...
.mdesk{height:100%;container-type:inline-size;display:flex;flex-direction:column;gap:7px;overflow:hidden;color:var(--ink);font-variant-numeric:tabular-nums}.mdesk *{box-sizing:border-box;min-width:0}.md-head{display:grid;grid-template-columns:1fr auto;gap:8px;align-items:center}.md-feed{display:flex;gap:7px;align-items:center;color:var(--muted);font-size:12px;min-width:0}.md-feed i{width:10px;height:10px;border-radius:50%;background:rgba(255,255,255,.18);box-shadow:0 0 0 7px rgba(255,255,255,.05);flex:0 0 auto}.md-feed i.on{background:#22d47b;box-shadow:0 0 0 7px rgba(34,212,123,.13),0 0 16px rgba(34,212,123,.32)}.md-feed b{font-size:11px;color:var(--ink);padding:4px 8px;border-radius:999px;background:rgba(255,255,255,.08);border:1px solid rgba(255,255,255,.10);white-space:nowrap}.md-refresh,.md-foot button{height:30px;border:1px solid rgba(255,255,255,.10);border-radius:999px;background:rgba(255,255,255,.09);color:var(--ink);font-weight:850;padding:0 12px;cursor:pointer}.md-refresh{width:34px;min-width:34px;padding:0;justify-self:end;align-self:center}.md-tabs{display:grid;grid-template-columns:repeat(4,1fr);gap:7px}.md-tabs button{height:38px;border:1px solid rgba(255,255,255,.10);border-radius:14px;background:linear-gradient(180deg,rgba(255,255,255,.075),rgba(255,255,255,.045));padding:6px 9px;display:grid;grid-template-columns:auto 1fr;gap:8px;align-items:center;cursor:pointer;overflow:hidden}.md-tabs strong{font-size:13px;color:var(--ink);white-space:nowrap}.md-tabs small{font-size:12px;font-weight:900;color:#25d889;text-align:right;overflow:hidden;text-overflow:ellipsis}.md-tabs .up small{color:#ff667c}.md-tabs .on{border-color:rgba(255,211,106,.48);background:linear-gradient(135deg,rgba(255,211,106,.18),rgba(34,212,123,.10))}.md-chart,.md-quote{border:1px solid rgba(255,255,255,.10);border-radius:18px;background:linear-gradient(180deg,rgba(255,255,255,.105),rgba(255,255,255,.052));padding:10px;overflow:hidden}.md-chart{flex:1 1 auto;min-height:0;display:flex;flex-direction:column}.md-title{display:flex;align-items:baseline;gap:8px;flex-wrap:wrap;margin-bottom:7px}.md-title em{font-style:normal;font-size:11px;font-weight:950;letter-spacing:.12em;color:#ffd36a}.md-title b{font-size:clamp(17px,4.2cqw,22px);line-height:1.1}.md-title span{font-size:12px;color:var(--muted);font-weight:850}.md-title.compact{margin-bottom:0}.md-tv{flex:1 1 auto;min-height:150px;border-radius:13px;overflow:hidden;background:#070d16;box-shadow:inset 0 0 0 1px rgba(255,255,255,.08)}.tradingview-widget-container,.tradingview-widget-container__widget{width:100%!important;height:100%!important}.md-loading,.md-error{height:100%;display:grid;place-items:center;color:var(--muted);font-size:13px;border:1px dashed rgba(255,255,255,.12);border-radius:14px;background:rgba(0,0,0,.18)}.md-quote{display:none;flex:0 0 auto}.md-quote-top{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:8px;align-items:start}.md-change{display:flex;flex-direction:column;gap:2px;text-align:right}.md-change b{font-size:14px;color:#22d47b}.up .md-change b{color:#ff667c}.md-price strong{display:block;margin-top:4px;font-size:clamp(30px,10cqw,42px);line-height:1;font-weight:950;color:#22d47b}.up .md-price strong{color:#ff667c}.md-range{display:grid;grid-template-columns:auto 1fr auto;gap:8px;align-items:center;margin-top:7px;color:var(--muted);font-size:11px}.md-range div{height:5px;border-radius:999px;position:relative;background:linear-gradient(90deg,rgba(34,212,123,.28),rgba(255,211,106,.32),rgba(255,102,124,.28))}.md-range i{position:absolute;top:50%;width:11px;height:11px;border-radius:50%;background:var(--ink);transform:translate(-50%,-50%)}.md-deals{display:grid;grid-template-columns:repeat(3,1fr);gap:6px;margin-top:8px}.md-deals div,.md-stats div{border:1px solid rgba(255,255,255,.08);border-radius:12px;background:rgba(0,0,0,.10);padding:7px 9px;overflow:hidden;min-height:46px}.md-deals span,.md-stats span{display:block;color:var(--muted);font-size:10px;letter-spacing:.08em;white-space:nowrap}.md-deals b,.md-stats b{display:block;margin-top:3px;font-size:15px;color:var(--ink);overflow:hidden;text-overflow:ellipsis}.md-stats{display:grid;grid-template-columns:repeat(2,1fr);gap:6px;margin-top:6px}.md-stats div{display:grid;grid-template-columns:auto 1fr;gap:1px 7px;align-items:center}.md-stats i{grid-row:span 2;width:22px;height:22px;border-radius:7px;display:grid;place-items:center;background:rgba(255,211,106,.12);color:#ffd36a;font-style:normal;font-size:9px;font-weight:900}.md-foot{display:flex;justify-content:flex-end;align-items:center;gap:8px;color:var(--muted);font-size:12px}.md-foot button{padding:5px 12px;height:auto;flex:0 0 auto}.chart-mode .md-chart{display:flex}.chart-mode .md-quote{display:none!important}.num-mode .md-chart{display:none!important}.num-mode .md-quote{display:block}.short .md-stats{grid-template-columns:repeat(2,1fr)}@container (max-width:520px){.md-tabs{grid-template-columns:repeat(2,1fr)}.md-tabs button{height:36px}.md-price strong{font-size:38px}.md-title b{font-size:19px}}@container (max-width:340px){.md-deals{grid-template-columns:1fr}.md-stats{grid-template-columns:1fr}.md-tabs small{font-size:11px}.md-foot{font-size:11px}.md-price strong{font-size:34px}}
`;document.head.appendChild(s);}

  function blank(p){return {id:p.id,name:p.name,short:p.short,url:p.url,unit:p.unit,ok:false,status:'',price:null,change:null,pct:null,buy:null,sell:null,high:null,low:null,prev:null,open:null,t:'--',sourceTime:null,source:'',ts:0};}
