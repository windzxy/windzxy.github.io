(()=>{
'use strict';
const VERSION='20260913-flight-chess-polish-v1.0';
...
function sync(app){const card=cardFor(app),state=card?.data?.flightChess;if(!state)return;const p=state.players?.[state.turn];if(p?.color)app.style.setProperty('--flight-accent',HEX[p.color]||'#3b82f6');decorateTrack(app);enhancePlanes(app);let live=app.querySelector('.flight-a11y-live');if(!live){live=document.createElement('div');live.className='flight-a11y-live';live.setAttribute('aria-live','polite');app.appendChild(live)}live.textContent=state.message||'';const side=app.querySelector('.flight-side');if(side&&state.winner&&!side.querySelector('.flight-winner-banner')){const b=document.createElement('div');b.className='flight-winner-banner';b.textContent='🏆 '+({red:'紅方',yellow:'黃方',blue:'藍方',green:'綠方'}[state.winner]||'玩家')+' 完成全部 4 架飛機！';side.prepend(b)}}
function scan(){document.querySelectorAll('.flight-app').forEach(sync)}
function boot(){style();scan();new MutationObserver(()=>requestAnimationFrame(scan)).observe(document.body,{childList:true,subtree:true});document.addEventListener('click',e=>{const r=e.target.closest?.('[data-flight-roll]');if(r){const app=r.closest('.flight-app');app?.classList.add('flight-rolling');setTimeout(()=>app?.classList.remove('flight-rolling'),520)}},true);document.addEventListener('keydown',e=>{if(!['Enter',' '].includes(e.key))return;const p=e.target.closest?.('.flight-plane.movable[data-flight-plane]');if(p){e.preventDefault();p.dispatchEvent(new MouseEvent('click',{bubbles:true,cancelable:true}))}},true)}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot,{once:true});else boot();
window.WebDeskFlightChessPolish={version:VERSION,features:['dice-animation','keyboard-accessibility','colored-track','shortcut-markers','mobile-touch-polish','winner-banner','live-status']};
})();