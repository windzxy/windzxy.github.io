(()=>{
'use strict';
if(window.__basicToolsUxV3)return;window.__basicToolsUxV3=1;
...
function textMeta(value){const text=String(value||'');const chars=text.length;const compact=text.replace(/\s/g,'').length;const lines=text?text.split(/\r?\n/).length:0;const words=(text.trim().match(/[A-Za-z0-9]+(?:['’-][A-Za-z0-9]+)?|[\u3400-\u9fff]/g)||[]).length;return {chars,compact,lines,words,bytes:byteSize(text)}}
function addStatus(input,html){const s=document.createElement('div');s.className='basic-tool-status';s.innerHTML=html;(input.closest('.app-panel')||input.parentElement)?.appendChild(s);return s}
function enhance(root){
 if(root.matches?.('.text-app')&&!root.dataset.ux3){
  const input=root.querySelector('textarea');if(input){root.dataset.ux3='1';const s=addStatus(input,'<strong>等待文字</strong><small>0 bytes</small>');const refresh=()=>{const m=textMeta(input.value);s.querySelector('strong').textContent=m.chars?`${m.chars} 字符 · ${m.words} 詞/字 · ${m.lines} 行`:'等待文字';s.querySelector('small').textContent=m.chars?`${m.compact} 非空白字符 · ${m.bytes} bytes`:'0 bytes'};input.addEventListener('input',refresh);refresh()}
 }
 if(root.matches?.('.table-app')&&!root.dataset.ux3){
  const input=root.querySelector('#appTableInput'),preview=root.querySelector('#tablePreview');if(!input||!preview)return;root.dataset.ux3='1';
 ...
 if(root.matches?.('.json-app')&&!root.dataset.ux3){
  const input=root.querySelector('#jsonInput'),format=root.querySelector('#jsonFormat');if(!input||!format)return;root.dataset.ux3='1';
