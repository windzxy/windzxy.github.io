(function(){
  'use strict';
  const VER='20260827-calendar-widget-v4-apple-widget-redesign';
  ...
  function longDate(d){return zh()?`${d.getFullYear()}年${d.getMonth()+1}月${d.getDate()}日 ${longWeek(d.getDay())}`:`${longWeek(d.getDay())}, ${monthName(d.getMonth())} ${d.getDate()}, ${d.getFullYear()}`;}
  ...
  function renderFullControls(d,mode){
    return `<section class="cal4-full-controls"><button type="button" data-cal4-nav="prev-year">«</button><button type="button" data-cal4-nav="prev-month">‹</button><button type="button" data-cal4-nav="today" class="today">${t('Today','今天')}</button><button type="button" data-cal4-nav="next-month">›</button><button type="button" data-cal4-nav="next-year">»</button><div class="cal4-mode"><button type="button" data-cal4-mode="month" class="${mode==='month'?'on':''}">${t('Month','月')}</button><button type="button" data-cal4-mode="year" class="${mode==='year'?'on':''}">${t('Year','年')}</button></div></section>`;
  }
  function renderDrawer(d,mode,open){
    const y=d.getFullYear(),m=d.getMonth(),day=d.getDate();
    return `<section class="cal4-drawer ${open?'open':''}" ${open?'':'hidden'}><div class="cal4-drawer-card"><header><b>${t('Jump to date','跳轉日期')}</b><button type="button" data-cal4-close>×</button></header><label class="cal4-year"><span>${t('Year','年')}</span><input data-cal4-year type="number" inputmode="numeric" min="1900" max="2100" value="${y}"></label><div class="cal4-label">${t('Month','月')}</div><div class="cal4-month-palette">${M_EN.map((_,i)=>`<button type="button" data-cal4-month="${i}" class="${i===m?'on':''}">${esc(monthName(i))}</button>`).join('')}</div><div class="cal4-label">${t('Day','日')}</div><div class="cal4-day-palette">${Array.from({length:dim(y,m)},(_,i)=>`<button type="button" data-cal4-day="${i+1}" class="${i+1===day?'on':''}">${i+1}</button>`).join('')}</div><div class="cal4-drawer-mode"><button type="button" data-cal4-mode="month" class="${mode==='month'?'on':''}">${t('Month view','月視圖')}</button><button type="button" data-cal4-mode="year" class="${mode==='year'?'on':''}">${t('Year view','年視圖')}</button></div></div></section>`;
  }
  ...
  function renderSide(d){
    ...
    return `<aside class="cal4-side"><section class="cal4-date-card"><div>${d.getDate()}</div><b>${esc(longDate(d))}</b><span>${t('Lunar','農曆')} · ${esc(lunarFull(d))}</span></section><section class="cal4-events"><h3>${t('This month','本月')}</h3>${list.length?list.map(it=>`<button type="button" data-cal4-jump="${it.date}"><b>${it.date.slice(5).replace('-','/')}</b><span>${esc(it.title)}</span><em>${esc(it.tag)}</em></button>`).join(''):`<p>${t('No official holiday / transfer records this month','本月無官方假期 / 調休記錄')}</p>`}</section></aside>`;
  }
  function renderInner(d,mode,drawer){
    return `<div class="cal4-app">${renderHeader(d)}${renderFullControls(d,mode)}${renderDrawer(d,mode,drawer)}<main class="cal4-body"><div class="cal4-primary">${mode==='year'?renderYearGrid(d):renderMonthGrid(d)}</div>${renderSide(d)}</main></div>`;
  }
...
.desktop-card.calendar-v4-card{min-width:300px!important;min-height:300px!important}.desktop-card.calendar-v4-card .card-body{padding:0!important;overflow:hidden!important}.desktop-card.calendar-v4-card .resize-grip{z-index:20}.calendar-v4{height:100%;min-height:0;color:#f7f9ff;font-family:inherit;overflow:hidden}.calendar-v4 *{box-sizing:border-box}.cal4-app{position:relative;height:100%;display:grid;grid-template-rows:auto auto minmax(0,1fr);gap:12px;padding:18px;background:radial-gradient(circle at 12% 0%,rgba(99,173,255,.12),transparent 34%),linear-gradient(145deg,rgba(255,255,255,.065),rgba(255,255,255,.02));overflow:hidden}.cal4-head{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:14px;align-items:start}.cal4-title span{display:block;letter-spacing:.22em;font-size:11px;opacity:.62;margin-bottom:4px}.cal4-title h2{margin:0;font-size:30px;line-height:1;font-weight:950;letter-spacing:-.04em}.cal4-title p{margin:7px 0 0;font-size:14px;opacity:.75;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.cal4-nav{display:flex;gap:8px}.cal4-nav button,.cal4-full-controls button,.cal4-mode button,.cal4-drawer-mode button{height:38px;border:1px solid rgba(255,255,255,.14);border-radius:14px;background:linear-gradient(180deg,rgba(255,255,255,.12),rgba(255,255,255,.055));color:#fff;font-weight:880;cursor:pointer;box-shadow:inset 0 1px rgba(255,255,255,.08)}.cal4-nav button{min-width:38px;padding:0 12px}.cal4-nav .today,.cal4-full-controls .today{background:linear-gradient(135deg,rgba(147,88,61,.92),rgba(115,94,66,.85));border-color:rgba(255,183,128,.22)}.cal4-full-controls{display:grid;grid-template-columns:42px 42px 100px 42px 42px minmax(140px,180px);gap:10px;align-items:center}.cal4-mode,.cal4-drawer-mode{display:grid;grid-template-columns:1fr 1fr;border:1px solid rgba(255,255,255,.13);border-radius:14px;overflow:hidden;background:rgba(15,18,28,.22)}.cal4-mode button,.cal4-drawer-mode button{height:38px;border:0;border-radius:0;background:transparent}.cal4-mode button.on,.cal4-drawer-mode button.on{background:linear-gradient(135deg,rgba(139,82,62,.92),rgba(100,72,59,.82))}.cal4-body{min-height:0;display:grid;grid-template-columns:minmax(0,1fr) minmax(220px,28%);gap:14px}.cal4-primary{min-height:0;overflow:hidden}.cal4-month{height:100%;min-height:0;display:grid;grid-template-rows:auto minmax(0,1fr);gap:8px}.cal4-week,.cal4-grid{display:grid;grid-template-columns:repeat(7,minmax(0,1fr));gap:8px}.cal4-week span{text-align:center;font-size:12px;font-weight:850;opacity:.62}.cal4-grid{min-height:0;grid-template-rows:repeat(6,minmax(0,1fr))}.cal4-day{min-width:0;min-height:0;padding:8px;border:1px solid rgba(255,255,255,.10);border-radius:15px;background:linear-gradient(145deg,rgba(255,255,255,.10),rgba(255,255,255,.035));color:#f6f8ff;text-align:left;overflow:hidden;position:relative;cursor:pointer}.cal4-day strong{display:block;font-size:18px;line-height:1;font-weight:920}.cal4-day small{display:block;margin-top:5px;font-size:12px;line-height:1;font-weight:800;opacity:.66}.cal4-day.muted{opacity:.34}.cal4-day.weekend strong{color:#ff7f9d}.cal4-day.selected{border-color:#74f0ff;box-shadow:0 0 0 1px rgba(116,240,255,.46) inset;background:linear-gradient(135deg,rgba(153,92,62,.86),rgba(95,82,63,.68))}.cal4-day.is-today:not(.selected){border-color:rgba(255,186,104,.45)}.cal4-day i{position:absolute;right:8px;top:8px;width:5px;height:5px;border-radius:50%;background:#ffbd77}.cal4-side{min-height:0;display:grid;grid-template-rows:auto minmax(0,1fr);gap:12px}.cal4-date-card,.cal4-events{border:1px solid rgba(255,255,255,.12);border-radius:20px;background:linear-gradient(145deg,rgba(255,255,255,.095),rgba(255,255,255,.035));padding:16px}.cal4-date-card{display:grid;grid-template-columns:auto 1fr;column-gap:12px;align-items:center}.cal4-date-card div{grid-row:1/3;font-size:46px;font-weight:950;line-height:.9;letter-spacing:-.06em}.cal4-date-card b{font-size:15px;line-height:1.25}.cal4-date-card span{font-size:13px;opacity:.72}.cal4-events{min-height:0;overflow:auto}.cal4-events h3{margin:0 0 12px;font-size:17px}.cal4-events p{margin:0;padding:13px;border-radius:14px;background:rgba(255,255,255,.055);opacity:.72;line-height:1.45}.cal4-events button{width:100%;display:grid;grid-template-columns:46px 1fr auto;gap:8px;align-items:center;border:0;border-radius:13px;padding:9px;margin:0 0 8px;background:rgba(255,255,255,.06);color:#fff;text-align:left}.cal4-events b{color:#ffbd77}.cal4-events span{font-weight:800}.cal4-events em{font-style:normal;font-size:11px;opacity:.62}.cal4-year-grid{height:100%;overflow:auto;display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px}.cal4-mini{border:1px solid rgba(255,255,255,.11);border-radius:16px;padding:10px;background:rgba(255,255,255,.055);color:#fff;text-align:left}.cal4-mini>b{display:block;margin-bottom:8px}.cal4-mini>div{display:grid;grid-template-columns:repeat(7,1fr);gap:3px;text-align:center}.cal4-mini span{font-size:11px;font-weight:760;border-radius:5px}.cal4-mini span.muted{opacity:.25}.cal4-mini span.weekend{color:#ff7894}.cal4-mini span.selected{background:rgba(255,139,82,.78);color:#fff}.cal4-mini.current{border-color:rgba(116,240,255,.56)}.cal4-drawer{position:absolute;z-index:50;inset:0;background:rgba(9,12,18,.38);backdrop-filter:blur(8px);display:grid;place-items:start end;padding:14px}.cal4-drawer[hidden]{display:none!important}.cal4-drawer-card{width:min(360px,100%);max-height:100%;overflow:auto;border:1px solid rgba(255,255,255,.15);border-radius:20px;background:rgba(36,40,50,.98);box-shadow:0 26px 70px rgba(0,0,0,.44);padding:14px}.cal4-drawer-card header{display:flex;justify-content:space-between;align-items:center;margin-bottom:12px}.cal4-drawer-card header b{font-size:15px}.cal4-drawer-card header button{width:30px;height:30px;border:0;border-radius:10px;background:rgba(255,255,255,.08);color:#fff;font-size:18px}.cal4-year span,.cal4-label{display:block;font-size:12px;opacity:.68;margin:10px 0 6px}.cal4-year input{width:100%;height:38px;border:1px solid rgba(255,255,255,.13);border-radius:13px;background:rgba(255,255,255,.07);color:#fff;padding:0 12px;font-weight:850;outline:0}.cal4-month-palette{display:grid;grid-template-columns:repeat(3,1fr);gap:6px}.cal4-day-palette{display:grid;grid-template-columns:repeat(7,1fr);gap:6px}.cal4-month-palette button,.cal4-day-palette button{height:32px;border:0;border-radius:10px;background:rgba(255,255,255,.07);color:#fff;font-weight:820}.cal4-month-palette button.on,.cal4-day-palette button.on{background:linear-gradient(135deg,rgba(255,132,82,.8),rgba(255,198,94,.32))}.cal4-drawer-mode{margin-top:12px}.calendar-v4.cal4-widget .cal4-app{grid-template-rows:auto minmax(0,1fr);gap:9px;padding:12px}.calendar-v4.cal4-widget .cal4-full-controls,.calendar-v4.cal4-widget .cal4-side{display:none}.calendar-v4.cal4-widget .cal4-body{grid-template-columns:1fr;gap:0}.calendar-v4.cal4-widget .cal4-head{grid-template-columns:1fr auto;gap:8px;align-items:center}.calendar-v4.cal4-widget .cal4-title span{font-size:9px;margin-bottom:2px}.calendar-v4.cal4-widget .cal4-title h2{font-size:22px}.calendar-v4.cal4-widget .cal4-title p{font-size:12px;margin-top:4px}.calendar-v4.cal4-widget .cal4-nav{gap:5px}.calendar-v4.cal4-widget .cal4-nav button{height:31px;min-width:31px;padding:0 8px;border-radius:11px;font-size:12px}.calendar-v4.cal4-widget .cal4-nav .today{min-width:54px}.calendar-v4.cal4-widget .cal4-week,.calendar-v4.cal4-widget .cal4-grid{gap:5px}.calendar-v4.cal4-widget .cal4-week span{font-size:11px}.calendar-v4.cal4-widget .cal4-day{border-radius:11px;padding:6px}.calendar-v4.cal4-widget .cal4-day strong{font-size:15px}.calendar-v4.cal4-widget .cal4-day small{font-size:10px;margin-top:3px}.calendar-v4.cal4-widget .cal4-year-grid{grid-template-columns:repeat(2,1fr);gap:8px}.calendar-v4.cal4-tiny .cal4-app{padding:10px;gap:7px}.calendar-v4.cal4-tiny .cal4-title h2{font-size:20px}.calendar-v4.cal4-tiny .cal4-title span{display:none}.calendar-v4.cal4-tiny .cal4-title p{font-size:11px}.calendar-v4.cal4-tiny .cal4-nav button{height:28px;min-width:28px;padding:0 6px}.calendar-v4.cal4-tiny .cal4-nav .today{min-width:45px}.calendar-v4.cal4-tiny .cal4-week,.calendar-v4.cal4-tiny .cal4-grid{gap:4px}.calendar-v4.cal4-tiny .cal4-day{padding:4px;border-radius:9px}.calendar-v4.cal4-tiny .cal4-day strong{font-size:13px}.calendar-v4.cal4-tiny .cal4-day small{font-size:9px}.calendar-v4.cal4-short .cal4-title p{display:none}.calendar-v4.cal4-short .cal4-week,.calendar-v4.cal4-short .cal4-grid{gap:4px}.calendar-v4.cal4-short .cal4-day{padding:4px}.calendar-v4.cal4-short .cal4-day small{font-size:9px}.calendar-v4.cal4-wide .cal4-app{padding:24px;gap:14px}.calendar-v4.cal4-wide .cal4-title h2{font-size:38px}.calendar-v4.cal4-wide .cal4-grid{gap:10px}.calendar-v4.cal4-wide .cal4-day strong{font-size:22px}.calendar-v4.cal4-wide .cal4-day small{font-size:14px}@media(max-width:520px){.calendar-v4 .cal4-nav button[data-cal4-drawer]{display:inline-flex}.calendar-v4 .cal4-drawer{padding:10px}.calendar-v4 .cal4-drawer-card{width:100%}}[data-theme="light"] .calendar-v4{color:#172033}[data-theme="light"] .cal4-app{background:linear-gradient(145deg,rgba(255,255,255,.76),rgba(255,255,255,.45))}[data-theme="light"] .cal4-nav button,[data-theme="light"] .cal4-full-controls button,[data-theme="light"] .cal4-mode,[data-theme="light"] .cal4-day,[data-theme="light"] .cal4-side,[data-theme="light"] .cal4-date-card,[data-theme="light"] .cal4-events,[data-theme="light"] .cal4-drawer-card{color:#172033;border-color:rgba(15,23,42,.13);background:rgba(255,255,255,.62)}[data-theme="light"] .cal4-drawer-card input,[data-theme="light"] .cal4-month-palette button,[data-theme="light"] .cal4-day-palette button{color:#172033;background:rgba(15,23,42,.06)}
`;document.head.appendChild(st);
  ...
  function onClick(e){
    ...
    const nav=e.target.closest('[data-cal4-nav]');if(nav){const a=nav.dataset.cal4Nav;jump(root,{today:a==='today',addMonth:a==='prev-month'?-1:a==='next-month'?1:0,addYear:a==='prev-year'?-1:a==='next-year'?1:0});return;}
