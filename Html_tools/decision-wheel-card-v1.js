(()=>{
  ...
  const VERSION='20260909-decision-wheel-v1.3';
  ...
  function installStyles(){
    ...
    style.textContent=`
      ...
      .spend-wheel-widget{height:100%;display:flex;flex-direction:column;align-items:center;gap:12px;padding:10px 14px 16px;box-sizing:border-box;overflow:auto}
      .spend-wheel-question{font-size:20px;font-weight:800;letter-spacing:.02em;text-align:center}
      .spend-wheel-stage{position:relative;width:clamp(260px,min(52vh,72vw),420px);aspect-ratio:1;display:grid;place-items:center;flex:0 0 auto}
      .spend-wheel-pointer{position:absolute;z-index:4;top:-10px;left:50%;transform:translateX(-50%);font-size:28px;filter:drop-shadow(0 2px 3px rgba(0,0,0,.32))}
      .spend-wheel-disk{position:relative;width:100%;height:100%;border-radius:50%;background:conic-gradient(var(--wheel-segments));border:8px solid rgba(255,255,255,.84);box-shadow:0 14px 38px rgba(0,0,0,.24),inset 0 0 0 1px rgba(0,0,0,.07);transform:rotate(var(--wheel-rotation));transition:transform 3.2s cubic-bezier(.12,.7,.08,1);will-change:transform}
      ...
      .spend-wheel-hub span{font-size:11px;opacity:.62}.spend-wheel-hub strong{font-size:clamp(20px,3vw,30px);line-height:1.1}
      .spend-wheel-result{text-align:center;display:grid;gap:3px}.spend-wheel-result strong{font-size:20px}.spend-wheel-result small{opacity:.65}
      .spend-wheel-spin{width:min(420px,100%);min-height:46px;border:0;border-radius:14px;font-weight:800;font-size:16px;background:linear-gradient(135deg,#b9e781,#86c84c);color:#18300d;box-shadow:0 8px 18px rgba(95,150,45,.2);cursor:pointer}
      .spend-wheel-spin:disabled{opacity:.6;cursor:wait}.spend-wheel-spin:active{transform:translateY(1px)}
      .spend-wheel-settings{width:min(420px,100%);font-size:12px}.spend-wheel-settings summary{cursor:pointer;text-align:center;opacity:.72;padding:4px}.spend-wheel-settings label{display:grid;grid-template-columns:1fr;gap:4px;margin:7px 0}.spend-wheel-settings input{width:100%;box-sizing:border-box;border-radius:9px;border:1px solid rgba(127,127,127,.25);padding:8px 10px;background:rgba(255,255,255,.12);color:inherit}.spend-wheel-budget{text-align:center;opacity:.74;padding:4px}
      @media(max-width:520px){.spend-wheel-widget{gap:9px;padding:6px 10px 12px}.spend-wheel-question{font-size:18px}.spend-wheel-stage{width:min(250px,76vw)}.spend-wheel-label{font-size:10px}.spend-wheel-pointer{font-size:24px}.desktop-card.t-spend-wheel{min-width:290px}.spend-wheel-spin,.spend-wheel-settings{width:min(320px,100%)}}
