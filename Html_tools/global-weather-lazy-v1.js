(()=>{
'use strict';
const VERSION='20260911-global-weather-lazy-v1.0';
...
const SCRIPTS=[
 ...
 ['Html_tools/typhoon-weather-motion-v1.js','20260910-typhoon-weather-motion-v1.5-pan-swr'],
 ['Html_tools/typhoon-zoom-motion-v2-1.js','20260910-typhoon-zoom-motion-v2.1-real-vectors'],
 ['Html_tools/typhoon-real-weather-v5-1.js','20260910-typhoon-real-weather-v5.1-independent-scalars'],
...
async function activate(){if(loading||loaded||!hasWeather())return;loading=true;observer?.disconnect();observer=null;for(let i=0;i<SCRIPTS.length;i++){const [src,v]=SCRIPTS[i];await load(src,v);if(i===7||i===12)await idle()}loaded=true;loading=false;window.dispatchEvent(new CustomEvent('webdesk-global-weather-ready'))}
...
window.WebDeskGlobalWeatherLazy={version:VERSION,get loaded(){return loaded},activate,scripts:SCRIPTS.map(x=>x[0])};
})();