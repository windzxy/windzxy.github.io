(()=>{
'use strict';
const VERSION='20260911-global-weather-lazy-v1.4-smooth-forecast-panzoom';
...
async function activate(){if(loading||loaded||!hasWeather())return;loading=true;observer?.disconnect();observer=null;for(let i=0;i<SCRIPTS.length;i++){const [src,v]=SCRIPTS[i];await load(src,v);if(i===8||i===12)await idle()}loaded=true;loading=false;window.dispatchEvent(new CustomEvent('webdesk-global-weather-ready'))}
...
window.WebDeskGlobalWeatherLazy={version:VERSION,get loaded(){return loaded},activate,scripts:SCRIPTS.map(x=>x[0]),performanceProfile:'cached-debounced-forecast-panzoom+single-weather-animation-runtime+provider-failover'};
})();