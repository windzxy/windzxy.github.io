(function(){
  'use strict';
  if(window.__windzxyCalendarV3LitePolishLoaded)return;
  window.__windzxyCalendarV3LitePolishLoaded=1;
  const VER='20260827-calendar-v3-lite-polish1-fast-ui';
  const MONTHS={
  ...
  function monthNames(){return MONTHS[lang()]||MONTHS['zh-HK'];}
  function esc(s){return String(s??'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]));}
  function installStyle(){
    if(document.getElementById('windzxyCalendarV3LitePolishStyle'))return;
    const s=document.createElement('style');
    s.id='windzxyCalendarV3LitePolishStyle';
    s.textContent=`
.calendar-v3.cv3-lite-polish{gap:9px!important;min-height:0!important;overflow:hidden!important}.calendar-v3.cv3-lite-polish .cv3-foot{display:none!important}.calendar-v3.cv3-lite-polish .cv3-title strong{font-size:clamp(20px,2.8vw,28px)!important}.calendar-v3.cv3-lite-polish .cv3-title em{white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:100%}.calendar-v3.cv3-lite-polish .cv3-body,.calendar-v3.cv3-lite-polish .cv3-year-view,.calendar-v3.cv3-lite-polish .cv3-month{min-width:0!important;min-height:0!important}.calendar-v3.cv3-lite-polish .cv3-events{box-sizing:border-box!important;z-index:0!important}.calendar-v3.cv3-lite-polish .cv3-day{box-sizing:border-box!important;min-width:0!important}.calendar-v3.cv3-lite-polish .cv3-day div{display:none!important}
.calendar-v3.cv3-lite-polish select[data-cv3-month]{position:absolute!important;width:1px!important;height:1px!important;opacity:0!important;pointer-events:none!important}.calendar-v3 .cv3-month-select-shell{position:relative;width:100%;min-width:0}.calendar-v3 .cv3-month-display{width:100%!important;height:40px!important;display:flex!important;align-items:center!important;justify-content:space-between!important;gap:8px!important;padding:0 14px!important;border:1px solid rgba(255,255,255,.16)!important;border-radius:14px!important;background:linear-gradient(180deg,rgba(255,255,255,.13),rgba(255,255,255,.075))!important;color:#f4f7fb!important;font:inherit!important;font-weight:700!important;text-align:left!important;box-sizing:border-box!important}.calendar-v3 .cv3-month-display:after{content:'⌄';opacity:.72}.calendar-v3 .cv3-month-display.is-open:after{content:'⌃'}
.calendar-v3 .cv3-month-menu{position:absolute;z-index:999999;left:0;right:0;top:calc(100% + 6px);max-height:min(310px,42vh);overflow:auto;padding:6px;border:1px solid rgba(255,255,255,.22);border-radius:16px;background:rgba(43,47,56,.98);box-shadow:0 18px 44px rgba(0,0,0,.42),inset 0 1px rgba(255,255,255,.08);backdrop-filter:blur(18px)}.calendar-v3 .cv3-month-menu[hidden]{display:none!important}.calendar-v3 .cv3-month-menu button{width:100%!important;height:34px!important;border:0!important;border-radius:11px!important;background:transparent!important;color:#f4f7fb!important;text-align:left!important;padding:0 12px!important;font-weight:650!important}.calendar-v3 .cv3-month-menu button:hover{background:rgba(255,255,255,.12)!important}.calendar-v3 .cv3-month-menu button.on{background:linear-gradient(135deg,rgba(255,122,60,.42),rgba(255,202,88,.22))!important;color:#fff!important}
[data-theme="light"] .calendar-v3 .cv3-month-display{background:rgba(255,255,255,.80)!important;color:#1f2937!important;border-color:rgba(15,23,42,.14)!important}[data-theme="light"] .calendar-v3 .cv3-month-menu{background:rgba(248,250,252,.98);border-color:rgba(15,23,42,.14);box-shadow:0 16px 36px rgba(15,23,42,.18)}[data-theme="light"] .calendar-v3 .cv3-month-menu button{color:#1f2937!important}
.calendar-v3 .cv3-year-view .cv3-mini.is-start-month{outline:1px solid rgba(102,224,255,.45);box-shadow:0 0 0 1px rgba(102,224,255,.10) inset}.calendar-v3 .cv3-year-view .cv3-mini.is-start-month h5:after{content:' · Current';font-size:10px;opacity:.58;font-weight:500}.calendar-v3:not([lang="en"]) .cv3-year-view .cv3-mini.is-start-month h5:after{content:' · 當月'}
...
`;
  ...
  function enhanceSelect(select){
    if(!select||select.dataset.cv3LiteSelect==='1')return;
    const parent=select.parentElement;if(!parent)return;
    select.dataset.cv3LiteSelect='1';
    ...
    function paint(){
      const list=monthNames();const val=Number(select.value||0);
    ...
    select.addEventListener('change',()=>{paint();scheduleScan(70);},true);
  }
  ...
  function classify(root){
    ...
    root.classList.toggle('cv3-w-small',w<690);root.classList.toggle('cv3-w-phone',w<540);root.classList.toggle('cv3-w-micro',w<430);root.classList.toggle('cv3-h-short',h<520);root.classList.toggle('cv3-h-tiny',h<430);
  ...
  function boot(){
    installStyle();scan();
    if(window.ResizeObserver&&!window.__windzxyCalendarV3LiteResizeObserver){
      const seen=new WeakSet();
      const ro=new ResizeObserver(entries=>entries.forEach(e=>classify(e.target)));
      window.__windzxyCalendarV3LiteResizeObserver=ro;
      const observeCalendars=()=>document.querySelectorAll('.calendar-v3').forEach(el=>{if(!seen.has(el)){seen.add(el);ro.observe(el);}});
      observeCalendars();
      const canvas=document.getElementById('desktopCanvas')||document.body;
      new MutationObserver(()=>{observeCalendars();scheduleScan(80);}).observe(canvas,{childList:true,subtree:true});
    }else{
      const canvas=document.getElementById('desktopCanvas')||document.body;
      new MutationObserver(()=>scheduleScan(120)).observe(canvas,{childList:true,subtree:true});
    }
    document.addEventListener('click',e=>{if(!e.target.closest('.cv3-month-select-shell'))closeMenus();if(e.target.closest('.calendar-v3'))scheduleScan(80);},true);
    document.addEventListener('change',e=>{if(e.target.closest('.calendar-v3'))scheduleScan(80);},true);
