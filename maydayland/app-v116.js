(() => {
  ...
  const DATA_URL = './data/product-v115.json?v=116.0.0';
  const ATLAS_URL = './atlas-v102.html?v=116.0.0';
  const $ = (s,r=document)=>r.querySelector(s);
  const esc = v => String(v ?? '').replace(/[&<>"']/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot',"'":'&#39;'}[c]));
  const fallback = {version:VERSION,cities:[],tours:[],albums:[],songCollections:[],books:[],timeline:[]};
  const state = {data:fallback,page:'home',tour:'all',city:'taipei',atlasMode:'shader',atlasProfile:'full',atlasReady:false};
  document.addEventListener('DOMContentLoaded', init);
  async function init(){
    try{const r=await fetch(DATA_URL,{cache:'no-store'}); if(r.ok) state.data=await r.json();}catch(e){console.warn('Maydayland v116 fallback',e)}
  ...
  function onAtlasMessage(e){if(e.origin!==location.origin)return;const m=e.data||{};if(!/^maydayland:atlas-/.test(m.type||''))return;if(m.type==='maydayland:atlas-ready'||m.type==='maydayland:atlas-profile-ack'||m.type==='maydayland:atlas-state'){state.atlasReady=true;document.documentElement.dataset.v116Atlas=m.certification||'ready';const badge=$('[data-atlas-status]');if(badge)badge.textContent=`ATLAS ${String(m.certification||'READY').toUpperCase()} · ${String(m.profile||state.atlasProfile).toUpperCase()}`;}}
  function sendAtlasProfile(){const f=$('.atlas-frame');if(!f?.contentWindow)return;try{f.contentWindow.postMessage({type:'maydayland:atlas-profile',profile:state.atlasProfile,reason:'v116-shell-adaptive',source:'maydayland-v116'},location.origin);f.contentWindow.postMessage({type:'maydayland:atlas-state-request'},location.origin)}catch{}}
  function render(){
    const root=$('#app'); if(!root)return;
    root.innerHTML = `<div class="shell">
      <header class="top">
        <a class="brand" href="#home"><img src="./mayday-logo.svg?v=116" alt="Maydayland"><span><b>MAYDAYLAND</b><small>PRODUCT SHELL · v116</small></span></a>
      ...
      ${hero()}
      ${page()}
      <footer class="footer">v116 保留 v115 的乾淨 A–F 產品殼，同時把既有 Shader route flow、Bloom / performance controls 以 Home-only adaptive Atlas 接回；弱裝置與 Reduced Motion 自動使用 Lite SVG。</footer>
    </div>`;
    bind();
  }
  function tab(id,label){return `<button class="${state.page===id?'active':''}" data-nav="${id}">${esc(label)}</button>`}
  function hero(){return `<section class="hero"><article class="card hero-main"><span class="kicker">APPROVED PRODUCT DIRECTION · ADAPTIVE ATLAS</span><h1>巡演一種顏色，城市一個故事。</h1><p>主入口維持乾淨產品化；首頁高性能裝置使用真正 Three.js Shader Atlas，Bloom、pixel ratio 與效能 profile 由既有 certified runtime 控制。非首頁不掛載 WebGL，手機省流量、弱裝置或 Reduced Motion 自動降為 Lite SVG。</p></article><aside class="card hero-side"><div class="metric"><small>Release</small><b>v116</b><small>adaptive Atlas restoration</small></div><div class="metric"><small>Atlas</small><b>${state.atlasMode==='shader'?'Shader':'Lite'}</b><small>${state.atlasProfile.toUpperCase()} performance profile</small></div><div class="metric"><small>Pages</small><b>A–F</b><small>Home-only WebGL lifecycle</small></div></aside></section>`}
