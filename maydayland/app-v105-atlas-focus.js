/* Maydayland · Atlas v105 focus runtime
 * Evidence-native single-city focus for verified tour layers.
 * Preserves reduced-motion, operational description, and v99 compatibility API.
 */
(() => {
  'use strict';
  ...
  let ariaDescription = document.getElementById('atlasOperationalDescription');
  if (!ariaDescription) {
    ariaDescription = document.createElement('p');
  ...
  const verifiedSingleCityLayers = Object.freeze({
    'just-rock-it-2015-budokan': {
      cityId: 'tokyo', city: '東京', venue: '日本武道館', tour: 'Just Rock It! 2015',
      dates: ['2015-08-28', '2015-08-29'], evidence: 'OFFICIAL PRIMARY'
    },
    'redna-2017-budokan': {
      cityId: 'tokyo', city: '東京', venue: '日本武道館', tour: 'Re:DNA 2017',
      dates: ['2017-02-03', '2017-02-04'], evidence: 'OFFICIAL PRIMARY'
    }
  });

  if (!document.getElementById('atlasFocusStyleV105')) {
    const focusStyle = document.createElement('style');
    focusStyle.id = 'atlasFocusStyleV105';
    focusStyle.textContent = `
      ...
      .v99-tour-evidence[hidden]{display:none}.v99-tour-evidence__eyebrow{font-size:10px;font-weight:800;letter-spacing:.13em;text-transform:uppercase;opacity:.68}.v99-tour-evidence__title{margin-top:4px;font-size:17px;font-weight:800;letter-spacing:-.02em}.v99-tour-evidence__venue{margin-top:2px;opacity:.8}.v99-tour-evidence__dates{display:flex;flex-wrap:wrap;gap:6px;margin-top:10px}.v99-tour-evidence__date{padding:4px 7px;border-radius:999px;background:rgba(255,255,255,.1);font-variant-numeric:tabular-nums}.v99-tour-evidence__note{margin-top:9px;font-size:11px;opacity:.62}
  ...
  let evidencePanel = document.getElementById('atlasTourEvidenceV99');
  if (!evidencePanel) {
  ...
  const activeTourId = () => document.querySelector('#tourLegend [data-tour].is-active')?.dataset.tour
    || document.querySelector('#tourLegend [data-tour][aria-pressed="true"]')?.dataset.tour || 'all';
  ...
  const renderTourFocus = () => {
    ...
    evidencePanel.innerHTML = `<div class="v99-tour-evidence__eyebrow">${layer.evidence} · VERIFIED SINGLE-CITY NODE</div><div class="v99-tour-evidence__title">${layer.tour}</div><div class="v99-tour-evidence__venue">${layer.venue} · ${layer.city}</div><div class="v99-tour-evidence__dates">${layer.dates.map(date=>`<span class="v99-tour-evidence__date">${date}</span>`).join('')}</div><div class="v99-tour-evidence__note">Only verified dates are surfaced. No cross-city flight segment is generated for a single-city evidence layer.</div>`;
  ...
  document.getElementById('tourLegend')?.addEventListener('click', event => {
    if (event.target.closest('[data-tour]')) queueMicrotask(updateDescription);
  });

  const api = Object.freeze({
    refresh: updateDescription,
    snapshot: () => ({ runtime:'MAYDAYLAND_ATLAS_V105', selectedTourId:activeTourId(), tourFocus:atlasStage.dataset.tourFocus || 'overview', focusCity:atlasStage.dataset.focusCity || null, effectsProfile:atlasStage.dataset.effectsProfile, evidencePanelVisible:!evidencePanel.hidden })
  });
  window.MAYDAYLAND_ATLAS_FOCUS_V105 = api;
  window.MAYDAYLAND_ATLAS_FOCUS_V99 = api;
  document.documentElement.dataset.v105AtlasFocus = 'ready';
  atlasStage.dataset.runtime = 'MAYDAYLAND_ATLAS_V105';
})();
