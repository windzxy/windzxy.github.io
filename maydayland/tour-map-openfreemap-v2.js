(()=>{
'use strict';
const VER='20260908-mayday-openfreemap-v2.2-world-wrap';
...
function addVector(style){if(!map||!window.L?.maplibreGL)return;removeRaster();const old=base;base=window.L.maplibreGL({style:OFM+style,pane:'tilePane',interactive:false,attributionControl:false,renderWorldCopies:true,fadeDuration:0,padding:.05});base.addTo(map);if(old&&old!==base)try{map.removeLayer(old)}catch(_){}try{map.attributionControl.addAttribution('OpenFreeMap © OpenMapTiles · OpenStreetMap')}catch(_){}}
