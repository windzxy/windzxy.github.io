(() => {
  ...
  const DATA_URL='./data/publications-v48.json?v=84.0.0';
  ...
  async function init(){
    ...
    window.addEventListener('hashchange',()=>setTimeout(enhanceWhenBooksVisible,0));
    window.addEventListener('maydayland:shell-rendered',()=>requestAnimationFrame(enhanceWhenBooksVisible));
  }
  function onClick(e){
  ...
  function enhanceWhenBooksVisible(){
    ...
    if(sec.dataset.v84BooksEnhanced==='1')return;
    sec.dataset.v84BooksEnhanced='1';
    render();
  }
  function render(){
...
(() => {
  'use strict';
  const VERSION='84.0.0';
  const DATA_URL='./data/timeline-v49.json?v=84.0.0';
  ...
  async function init(){
    ...
    window.addEventListener('maydayland:shell-rendered',()=>requestAnimationFrame(enhanceWhenVisible));
  }
  function onClick(e){
  ...
  function enhanceWhenVisible(){
    ...
    const sec=[...document.querySelectorAll('.v45-section')].find(s=>s.classList.contains('active'));
    if(!sec||sec.dataset.v84TimelineEnhanced==='1')return;
  ...
  function render(force=false,target=null){
    ...
    if(!force&&sec.dataset.v84TimelineEnhanced==='1')return;
    sec.dataset.v84TimelineEnhanced='1';
    const list=filtered();
...
document.documentElement.dataset.v84Archives='ready';
window.dispatchEvent(new CustomEvent('maydayland:archives-ready',{detail:{version:'v84',modules:['books','timeline'],lifecycle:'shell-rendered'}}));