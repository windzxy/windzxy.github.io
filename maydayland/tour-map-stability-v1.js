(()=>{
'use strict';
const VER='20260911-tour-map-stability-v1.0-mobile-blank-recovery';
...
function emitResize(delay=0){clearTimeout(resizeTimer);resizeTimer=setTimeout(()=>{window.dispatchEvent(new Event('resize'));requestAnimationFrame(()=>window.dispatchEvent(new Event('resize')))},delay)}
...
function recoverBlank(){if(!visibleMap()||hasPaint())return;const now=Date.now();if(now-lastRecovery<15000)return;lastRecovery=now;const current=activeMode(),all=buttons();const fallback=all.find(b=>b.dataset.mode===(current==='street'?'bright':'street'))||all.find(b=>b.dataset.mode!==current);if(!fallback)return;status('偵測到底圖空白，正在自動恢復…');fallback.click();setTimeout(()=>{const original=buttons().find(b=>b.dataset.mode===current);if(original)original.click();emitResize(120)},2600)}
function scheduleBlankCheck(delay=3800){clearTimeout(blankTimer);blankTimer=setTimeout(recoverBlank,delay)}
function rememberMode(e){const b=e.target?.closest?.('.mayday-map-modes button[data-mode]');if(!b)return;try{sessionStorage.setItem(KEY,b.dataset.mode)}catch(_){};scheduleBlankCheck()}
...
function mount(){const s=stage();if(!s)return false;emitResize(80);restoreMode();scheduleBlankCheck(4500);if('ResizeObserver'in window){observer?.disconnect?.();observer=new ResizeObserver(()=>{emitResize(80);scheduleBlankCheck(1200)});observer.observe(s)}return true}
document.addEventListener('click',rememberMode,true);
window.addEventListener('orientationchange',()=>{emitResize(180);scheduleBlankCheck(1600)});
document.addEventListener('visibilitychange',()=>{if(!document.hidden){emitResize(80);scheduleBlankCheck(1200)}});
window.addEventListener('pageshow',()=>{emitResize(80);scheduleBlankCheck(1400)});
