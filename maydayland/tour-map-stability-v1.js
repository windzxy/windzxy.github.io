(()=>{
'use strict';
const VER='20260913-tour-map-stability-v1.5-raster-owner-aware';
...
function isRasterMode(current){return current==='terrain'||current==='sat'}
function recoverBlank(){if(recovering)return;if(interactionBusy()){blankSince=0;scheduleBlankCheck(1100);return}if(!visibleMap()||hasPaint()){blankSince=0;return}const now=Date.now();if(!blankSince){blankSince=now;scheduleBlankCheck(900);return}const current=activeMode(),blankFor=now-blankSince;if(isRasterMode(current)&&blankFor<12000){emitResize(40);scheduleBlankCheck(1500);return}if(blankFor<850){scheduleBlankCheck(900);return}if(navigator.onLine===false){status('網絡暫時中斷，恢復連線後會自動重試',2400);scheduleBlankCheck(2600);return}if(now-lastRecovery<8000){scheduleBlankCheck(1200);return}lastRecovery=now;blankSince=0;const fallback=fallbackButton(current);if(!fallback)return;recovering=true;status(isRasterMode(current)?'底圖供應商恢復逾時，正在切換可用模式…':'底圖持續空白，正在切換可用備援…',2200);fallback.click();setTimeout(()=>{recovering=false;if(interactionBusy()){scheduleBlankCheck(1000);return}emitResize(40);if(hasPaint()){status('已切換至可正常顯示的備援底圖',1600);return}status('備援底圖仍未完成載入，繼續恢復…',1800);scheduleBlankCheck(900)},2100)}
