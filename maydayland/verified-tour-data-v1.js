...
function build(data){
  ...
  var knownShows=stops.reduce(function(total,stop){return total+(Number(stop.shows)||0);},0);
  var summary=document.createElement('p');summary.className='verified-summary';summary.textContent='目前已核實 '+stops.length+' 城'+(knownShows?' · '+knownShows+' 場':'');root.appendChild(summary);
  ...
  var note=document.createElement('p');note.className='verified-data-note';note.textContent='僅展示已由官方新聞頁明確支持的資料；地圖上的青色節點或場次卡片均可直接定位到對應城市。';root.appendChild(note);
  return root;
}
